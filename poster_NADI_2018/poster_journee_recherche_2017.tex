...
Feedback from users can be seen as \textbf{constraints} at different levels: Instance-level [A], Group-level [B], Feature-level [C] and Dataset-level [D].
...
\begin{multicols}{3}
    ...
    \section*{\small{Example-based constraints ([B],[C],[D])}}
    \begin{center}
        \begin{tabular}{c}
        ...
        \begin{tabular}{p{13em}}
            Using examples to guide the algorithm to construct the understandable axes.
        \end{tabular}
    \end{center}
\end{multicols}
}
...
\noindent
\begin{itemize}
    \item \textbf{Idea}: Extend t-SNE to take into account the user's feedback. The user moves points to control the groups:
    \begin{itemize}
        \compresslist
        \item Move points far apart to divide up a large cluster.
        \item Move points close together to merge the small similar clusters.
    \end{itemize}
\end{itemize}
\end{minipage}
