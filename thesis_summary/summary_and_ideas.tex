...
\subsection{Integrating pairwise constraints into $t$-SNE}
The advantage of $t$-SNE is to preserve the local structures and also the global structure of the data.
...
A question can be raised: do we need a $t$-SNE embedding that reflects well the size of groups and the relative distances between groups?
...
In fact it still takes time to make the inference procedure work in practice, but our main research problem is now how to model the users' feedback in a probabilistic framework.
...
We have proposed an idea of integrating the users' feedback on the position of points in the visualization into PPCA model.
...
In PPCA model, the data is described by a generative process, which is indeed a mapping function from a latent space to the observed data space.
...
We would like to use this covariance function to also encode the user constraints.
With this idea, we can model the observation-level interaction~\citep{endert2011observation} into GP-LVM model.
More specifically, our research question is how to present the constraints of the individual observations (fixed position of anchor points, pairwise constraints or constraints on group of points, etc.) by a covariance function of the Gaussian process models? 

\subsection{Interactive Visualization in Deep Learning Model}\label{sec:nn}
+ distilpub cite visualize features
+ viz pre-learned model (ICLR)
+ VAE~\underline{\citep{kingma2013auto}}
+ Semisupervised VAE~\citep{kingma2014semi}
